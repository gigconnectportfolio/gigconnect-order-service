...
const orderSchema: ObjectSchema = Joi.object().keys({
    ...
    // --- TRANSACTION FIELDS ---
    status: Joi.string().required(), // Stays as Joi.string().required()
    ...
    // **ADJUSTMENT:** Changed from required to optional/allow(null) for payment flexibility
    paymentIntent: Joi.string().optional().allow(null, ''),

    // Added to store payment-specific data, populated securely by the backend.
    flutterwave: Joi.object({
        // All fields are optional because the backend GENERATES/UPDATES them after validation.
        txRef: Joi.string().optional().allow(null, ''),
        transactionId: Joi.string().optional().allow(null, ''),
        gatewayStatus: Joi.string().optional().allow(null, ''),
        paymentMethod: Joi.string().optional().allow(null, ''),
        fee: Joi.number().optional().allow(null)
    }).optional(),

    // --- REMAINING FIELDS (UNCHANGED FROM YOUR ORIGINAL) ---
    ...
    deliveredWork: Joi.array()
        .items(Joi.object({
            message: Joi.string(), file: Joi.string()
        }))
        .optional(),
    ...
    buyerReview: Joi.object({
        rating: Joi.number(), review: Joi.string()
    }).optional(),
